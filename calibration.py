...
import math
import numpy as np
from receiveLaserData import tcpClient, udpClient
from plotPoints import point, point_270mini
...
def calibration_270mini(data):
    iHorizontalAngle = get_iHorizontalAngle_270mini(data)
    iHorizontalHeight = get_iHorizontalHeight_270mini(data, iHorizontalAngle)
    minDistance = get_minDistance_270mini(data, iHorizontalAngle, iHorizontalHeight)
    maxDistance = get_maxDistance(data, iHorizontalAngle, iHorizontalHeight, minDistance)
    return iHorizontalAngle, iHorizontalHeight, minDistance, maxDistance


def get_iHorizontalAngle_270mini(data):
    # 参与标定的帧数
    dataSize = len(data)
    thetaList = []
    # 获取对应距离的数据
    for idx, data_per_id in enumerate(data):
        usedData = []
        tanList = []
        for i, angleAndDistance in enumerate(data_per_id):
            angle = angleAndDistance[0]
            distance = angleAndDistance[1]
            if 1050 <= distance <= 2050:
                h = int(math.sin(math.radians(angle)) * distance)
                l = int(math.cos(math.fabs(angle) * math.pi / 180) * distance)
                usedData.append([l, h])
            else:
...
def get_iHorizontalHeight_270mini(data, iHorizontalAngle):
    dataSize = len(data)
    heightList = []
    for idx, data_per_idx in enumerate(data):
        usedData = []
        for i, angleAndDistance in enumerate(data_per_idx):
            angle0 = angleAndDistance[0]
            distance = angleAndDistance[1]
            # 保证1.2m-3.5m是路面
            if 2060 <= distance <= 3640:
                # angle0 = i * lidarAngleStep + lidarStartAngle
...
def get_minDistance_270mini(data, iHorizontalAngle, iHorizontalHeight):
    # 参与标定的帧数
    dataSize = len(data)
    minDistanceList = []
    # 获取对应距离的数据
...
if __name__ == '__main__':
    ...
    if lidarType == "dg270mini":
        data = udpClient()
        angle, height, minl, maxl = calibration_270mini(data)
    elif lidarType == "dg3000":
        data = tcpClient()
    ...
    point_270mini(data, angle, height)
